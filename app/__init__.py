...
@main_bp.route('/export/artifacts', methods=['GET'])
def export_artifacts():
    """
    Скачать список экспонатов в формате CSV
    ---
    tags:
      - Отчётность
    summary: Скачать отчет по экспонатам (CSV)
    description: Позволяет выгрузить всю базу экспонатов (approved и pending) в формате CSV для анализа или отчетности.
    produces:
      - text/csv
    responses:
      200:
    ...
    # Шапка
    writer.writerow([
        "ID", "Название", "Материалы", "Техника", "Год", "Описание", "Автор", "Категория", "Период", "Фото"
    ])
    for a in artifacts:
        writer.writerow([
            a.id,
...
def upload_file():
    ...
    if artifact and not artifact.photo_url:
        artifact.photo_url = f"/uploads/{filename}"
    db.session.commit()
...
def get_authors():
    """
    Получить список всех авторов с количеством экспонатов
    ---
    tags:
      - Авторы
    summary: Получить список всех авторов
    description: Возвращает массив всех авторов, добавленных в систему, и количество их экспонатов.
    responses:
      200:
    ...
    for author in authors:
        result.append({
            ...
            'artifacts_count': len(author.artifacts)  # <--- Вот тут считаем
        })
    return jsonify(result)
...
def get_artifacts():
    ...
    for artifact in artifacts:
        result.append({
            ...
            'applicant_id': artifact.applicant_id,
            'author_id': artifact.author_id,
            'author_full_name': artifact.author.full_name if artifact.author else None,  # <--- ВАЖНО!
            'category_id': artifact.category_id,
            'period_id': artifact.period_id,
            'geography_id': artifact.geography_id
        })
...
def get_artifact(artifact_id):
    ...
    result = {
        ...
        'applicant_id': artifact.applicant_id,
        'author_id': artifact.author_id,
        'author_full_name': artifact.author.full_name if artifact.author else None,
        'category_id': artifact.category_id,
        'period_id': artifact.period_id,
        'geography_id': artifact.geography_id,
        'files': files
...
def create_artifact():
    ...
    if author_data and author_data.get('full_name'):
        # Поиск по имени и (опционально) доп. полям
        ...
        if not author:
            author = Author(
                ...
                place_of_residence=author_data.get('place_of_residence'),
                additional_info=author_data.get('additional_info')
            )
            db.session.add(author)
            db.session.commit()
        author_id = author.id
    ...
    if applicant_data and applicant_data.get('full_name'):
        applicant = Applicant.query.filter_by(full_name=applicant_data['full_name'], email=applicant_data.get('email')).first()
        if not applicant:
            applicant = Applicant(
